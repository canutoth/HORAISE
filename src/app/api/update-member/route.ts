...
// Helper to escape sheet names with special characters
function escapeSheetName(name: string): string {
  // If the sheet name contains spaces, special chars, wrap in single quotes
  // and escape any single quotes inside by doubling them
  if (/[' \-\u00C0-\u017F]/.test(name)) {
...
async function findMemberRow(
  ...
  const escapedSheetName = escapeSheetName(SHEET_NAME);
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${escapedSheetName}!A:N`,
  });
  ...
  try {
    ...
    if (isNew) {
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `${escapedSheetName}!A${rowNumber}:N${rowNumber}`,
        valueInputOption: "RAW",
        requestBody: {
          values: [rowData],
        },
