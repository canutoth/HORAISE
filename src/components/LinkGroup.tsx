"use client";

import React from "react";
import { Group, ActionIcon } from "@mantine/core";
import {
  IconBrandLinkedin,
  IconBrandGithub,
  IconSchool,
  IconWorld,
} from "@tabler/icons-react";

interface SocialLinks {
  linkedin?: string;
  github?: string;
  googleScholar?: string;
  orcid?: string;
  lattes?: string;
  personalWebsite?: string;
}

interface LinkGroupProps {
  links: SocialLinks;
  isMobile?: boolean;
  align?: "left" | "center" | "right";
}

// Ícone customizado ORCID (ID)
const OrcidIcon = ({ size = 26 }: { size?: number }) => (
  <span
    aria-hidden
    style={{
      display: "inline-flex",
      alignItems: "center",
      justifyContent: "center",
      width: size ?? 26,
      height: size ?? 26,
      fontSize: (size ?? 26) * 0.7,
      fontWeight: 700,
      color: "currentColor",
      lineHeight: 1,
    }}
  >
    ID
  </span>
);

// Ícone customizado Lattes (SVG)
const LattesIcon = ({ size = 26 }: { size?: number }) => (
  <svg
    width={size ?? 26}
    height={size ?? 26}
    viewBox="0 0 448 512"
    xmlns="http://www.w3.org/2000/svg"
    role="img"
    aria-label="Lattes"
  >
    <path
      d="M 97.871854,434.73261 C 51.534463,339.78442 23.965602,282.44369 23.965602,281.02029 c 0,-2.32214 2.831558,-1.99974 30.672084,3.45957 48.965204,9.61389 75.126384,12.32631 118.735104,12.34258 57.69707,0.0159 104.6807,-9.1222 141.18473,-27.4842 19.31194,-9.71476 30.92555,-18.32755 40.43708,-29.99337 11.716,-14.37824 15.47977,-24.28004 15.61512,-40.94646 0.11867,-15.85237 -2.01801,-24.21167 -11.19035,-43.60874 -3.62892,-7.66433 -6.8168,-16.46265 -7.12098,-19.54964 -0.47493,-4.96814 -0.0684,-5.68084 3.59445,-6.10361 8.00292,-0.94846 47.50732,37.40224 62.05491,60.24069 25.07592,39.38574 27.11161,81.99337 5.88408,123.1953 -13.03903,25.31314 -27.44972,42.82712 -51.57723,62.73362 -40.09844,33.06211 -86.70754,56.08608 -151.06833,74.63514 C 186.61557,459.91141 130.71496,472 119.20225,472 c -2.44075,0 -7.02006,-8.00296 -21.295953,-37.28315 l -0.03402,0.0151 z M 110.77601,281.61191 C 65.760136,275.77998 27.985273,270.70947 26.81537,270.33687 24.815625,269.6926 17.660677,245.82107 13.624773,226.39004 12.607902,221.4726 11.11559,208.45131 10.30202,197.43174 6.6716589,148.26132 17.370799,114.26648 46.041165,83.697237 94.583571,31.98518 198.51713,25.694031 315.77765,67.369458 c 20.58274,7.324215 28.75504,12.410983 24.975,15.580668 -2.79708,2.339846 -21.75315,2.305883 -54.50916,-0.102387 -51.20464,-3.763759 -90.18335,3.357226 -110.27491,20.176211 -30.58742,25.60158 -25.92345,81.72365 13.53071,162.68196 4.27316,8.76586 8.57881,17.34466 9.56318,19.09094 2.28966,4.01773 0.62803,7.74899 -3.3572,7.56196 -1.69755,-0.0813 -39.91486,-4.91203 -84.92926,-10.74592 z m 151.01614,-44.04726 c -35.92814,-6.45997 -68.22691,-28.7388 -78.65437,-54.22127 -5.00209,-12.24165 -4.76437,-28.2131 0.57585,-37.77483 4.83279,-8.64723 17.3107,-18.64993 28.48481,-22.83843 18.59924,-6.96791 51.17019,-4.18853 74.90688,6.40975 22.53229,10.05487 42.50672,27.73816 49.93183,44.18457 9.52925,21.10841 1.59321,44.65955 -18.82072,55.90059 -13.5307,7.44285 -39.82676,11.32572 -56.44249,8.34109 h 0.0181 z"
      fill="currentColor"
    />
  </svg>
);

const LINK_CONFIGS = {
  linkedin: {
    icon: IconBrandLinkedin,
    hoverShadow: "0 4px 12px rgba(0, 119, 181, 0.3)",
    color: "blue" as const,
    aria: "LinkedIn",
  },
  github: {
    icon: IconBrandGithub,
    hoverShadow: "0 4px 12px rgba(139, 92, 246, 0.18)",
    color: "violet" as const,
    aria: "GitHub",
  },
  googleScholar: {
    icon: IconSchool,
    hoverShadow: "0 4px 12px rgba(66, 133, 244, 0.3)",
    color: "blue" as const,
    aria: "Google Scholar",
  },
  orcid: {
    icon: OrcidIcon,
    hoverShadow: "0 4px 12px rgba(166, 206, 57, 0.3)",
    color: "green" as const,
    aria: "ORCID",
  },
  lattes: {
    icon: LattesIcon,
    hoverShadow: "0 4px 12px rgba(37, 99, 235, 0.18)",
    color: "blue" as const,
    aria: "Lattes",
  },
  personalWebsite: {
    icon: IconWorld,
    hoverShadow: "0 4px 12px rgba(249, 115, 22, 0.18)",
    color: "orange" as const,
    aria: "Website",
  },
};

export const LinkGroup: React.FC<LinkGroupProps> = ({
  links,
  isMobile = false,
  align = "center",
}) => {
  const justifyMap = {
    left: "flex-start",
    center: "center",
    right: "flex-end",
  };

  const iconSize = isMobile ? 22 : 26;
  const actionIconSize = isMobile ? "lg" : "xl";

  const linkItems = Object.entries(links)
    .filter(([_, url]) => url && url.trim() !== "")
    .map(([type, url]) => {
      const config = LINK_CONFIGS[type as keyof typeof LINK_CONFIGS];
      if (!config) return null;

      const IconComponent = config.icon;

      return {
        href: url,
        icon: <IconComponent size={iconSize} />,
        hoverShadow: config.hoverShadow,
        color: config.color,
        aria: config.aria,
      };
    })
    .filter(Boolean);

  if (linkItems.length === 0) return null;

  return (
    <Group gap="sm" justify={justifyMap[align]}>
      {linkItems.map((item, idx) => (
        <ActionIcon
          key={idx}
          component="a"
          href={item!.href}
          target="_blank"
          rel="noopener noreferrer"
          variant="light"
          color={item!.color}
          size={actionIconSize}
          // Use a large radius so the ActionIcon appears fully rounded
          radius="xl"
          style={{
            transition: "box-shadow 0.18s ease, transform 0.18s ease",
            // explicit borderRadius to ensure consistent rounding across browsers
            borderRadius: 9999,
            // clip content to the rounded shape
            overflow: "hidden",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.boxShadow = item!.hoverShadow;
            e.currentTarget.style.transform = "translateY(-2px)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.boxShadow = "";
            e.currentTarget.style.transform = "";
          }}
          aria-label={item!.aria}
        >
          {item!.icon}
        </ActionIcon>
      ))}
    </Group>
  );
};

export default LinkGroup;
